...
echo "INDY_NODE_LOG_LEVEL=${INDY_NODE_LOG_LEVEL:=0}"
echo "INDY_NODE_LOG_STDOUT=${INDY_NODE_LOG_STDOUT:=True}"
echo "INDY_NODE_LOG_DIR=${INDY_NODE_LOG_DIR:=/var/log/indy}"
...
awk '{if (index($1, "enableStdOutLogging") != 0) {print("enableStdOutLogging = '$INDY_NODE_LOG_STDOUT'")} else print($0)}' /etc/indy/indy_config.py> /tmp/indy_config.py
mv /tmp/indy_config.py /etc/indy/indy_config.py

awk '{if (index($1, "LOG_DIR") != 0) {print("LOG_DIR = \"'$INDY_NODE_LOG_DIR'\"")} else print($0)}' /etc/indy/indy_config.py> /tmp/indy_config.py
mv /tmp/indy_config.py /etc/indy/indy_config.py

# Append logLevel key if not exists and set it with value from env
grep -qxF 'logLevel"' /etc/indy/indy_config.py || printf "\n# Defines the LogLevel of IndyNode \nlogLevel = %s\n" "$INDY_NODE_LOG_LEVEL" >> /etc/indy/indy_config.py
